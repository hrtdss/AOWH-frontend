...
const RequireAuth = ({ allowedPages }) => { 
    ...
    if (userData) {
        return Object.keys(userData).some(role => (allowedPages.includes(role) && userData[role])) ? <Outlet/> : <Navigate to='/AOWH-frontend' state={{ from: location }}/>;
    }

    return <Navigate to='/AOWH-frontend' state={{ from: location }}/>;
